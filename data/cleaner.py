...
subs = pd.read_csv(sys.argv[1])

for vin in subs['SHA1 of VIN'].unique():
    (dataroot / vin).mkdir(exist_ok=True)

input_files = []
for i, row in subs.iterrows():
    for url in row['Upload CSV files from Car Scanner'].split(','):
        csv_filename = parse.urlparse(url).path.split('/')[-1]
        filepath: Path = dataroot / row['SHA1 of VIN'] / csv_filename
        if not filepath.is_file():
            request.urlretrieve(url, filepath)
            print(f"Received {url} at {row['SHA1 of VIN']}")
        else:
            print(f"Skipped {url} (already exists)")
        if not filepath.with_name("cleaned-" + filepath.name).is_file():
...
for input_file in input_files:
    ...
    for distance in distances:
        standardize_unit(df_notimedelta, distance, {"miles": lambda s: s * 1.609}, "km")
    
    powers = [
        "HV EV Battery Power",
        "DC Charger Power",
        "Hybrid/EV Charger Power"
    ]
    for power in powers:
        standardize_unit(df_notimedelta, power, {"hp": lambda s: s / 1.341}, "kW")

    df_notimedelta['12V device load (A)'] = df_notimedelta['[ICCU] LDC Output Current (A)'] - df_notimedelta['[ICCU] Aux. Battery Current (A)']
    df_notimedelta.to_csv(str(input_file.with_name("cleaned-" + input_file.name)), index=False)
